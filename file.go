...
type fileLogger struct {
	...
	DayMaxReserve int    `json:"day_max_reserve"`
...
func (f *fileLogger) createFreshFile(logTime time.Time) error {
	...
	if f.dailyOpenDate != logTime.Day() {
		for ; err == nil && num <= f.DayMaxReserve; num++ {
	...
	if err == nil {
		// find no log number
		// delete latest, then reuse
		err := os.Remove(f.Filename)
		if err != nil {
			return fmt.Errorf("Cannot find free log number to rename %s, but delete it still fail: %v", f.Filename, err)
		}
		f.fileWriter.Close()
		goto RESTART_LOGGER
	}
